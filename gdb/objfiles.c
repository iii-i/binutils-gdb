...
static int insert_section_p (const struct bfd *, const struct bfd_section *);
...
/* Section map is an interval tree containing sections from all objfiles -
   including the overlapping ones.  */
...
struct section_map_entry
{
  ...
  ~section_map_entry ()
  {
    gdb_assert (section->section_map_entry == this);
    section->section_map_entry = nullptr;
  }
...
/* Add OBJFILE's sections to the section map.  */

void
objfile_build_section_map (objfile *objfile)
{
  objfile_pspace_info *pspace_info = get_objfile_pspace_data (objfile->pspace);
  obj_section *s;
...
/* Remove OBJFILE's sections from the section map.  */

void
objfile_clear_section_map (objfile *objfile)
{
  ...
  ALL_OBJFILE_OSECTIONS (objfile, s)
      ...
      if (s->section_map_entry != nullptr)
	{
	  ...
	  info->sections.erase (
	      interval_tree<section_map_entry>::iterator::from_interval (
		  info->sections, s->section_map_entry));
	}
    }
}
...
objfile::~objfile ()
{
  ...
  objfile_clear_section_map (this);
  ...
  ALL_OBJFILE_OSECTIONS (objfile, s)
    {
      ...
      if (s->section_map_entry != nullptr)
	{
	  pspace_info->sections.erase (
	      interval_tree<section_map_entry>::iterator::from_interval (
		  pspace_info->sections, s->section_map_entry));
	}
      if (insert_section_p (objfile->obfd, s->the_bfd_section))
	pspace_info->sections.emplace (s);
